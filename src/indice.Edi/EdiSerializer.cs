...
public class EdiSerializer
{
    ...
    public object Deserialize(EdiReader reader) => Deserialize(reader, null);
    ...
    public T Deserialize<T>(TextReader reader, IEdiGrammar grammar) => (T)Deserialize(reader, grammar, typeof(T));
    ...
    public object Deserialize(TextReader reader, IEdiGrammar grammar, Type objectType) => Deserialize(new EdiTextReader(reader, grammar), objectType);
    ...
    /// <typeparam name="T">The type of the object to deserialize.</typeparam>
    /// <returns>The instance of <typeparamref name="T"/> being deserialized.</returns>
    public T Deserialize<T>(EdiReader reader) => (T)Deserialize(reader, typeof(T));
    ...
    /// <param name="objectType">The <see cref="Type"/> of object being deserialized.</param>
    /// <returns>The instance of <paramref name="objectType"/> being deserialized.</returns>
    public object Deserialize(EdiReader reader, Type objectType) => DeserializeInternal(reader, objectType);
    ...
    private static void SerializeStructure(EdiWriter writer, Stack<EdiStructure> stack, EdiPathComparer structuralComparer = null) {
        ...
        foreach (var property in properies) {
            ...
            if (property.ValueInfo != null) {
                ...
                // handle auto generated values. Control counts and stuff.
                if (property.CountInfo != null) {
                    switch (property.CountInfo.Scope) {
                        case EdiCountScope.Segments:
                            value = writer.SegmentCount;
                            break;
                        case EdiCountScope.Messages:
                            value = writer.MessageCount;
                            break;
                        case EdiCountScope.Groups:
                            value = writer.GroupCount;
                            break;
                    }
                }
